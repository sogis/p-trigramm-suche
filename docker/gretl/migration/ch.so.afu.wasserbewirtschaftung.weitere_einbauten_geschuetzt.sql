...
index_base AS (
    SELECT
        'ch.so.afu.wasserbewirtschaftung.weitere_einbauten_geschuetzt'::text AS subclass,
        t_id AS id_in_class,
        concat('Nr: ', objektnummer, ' (VEGAS Einbauten)')  AS displaytext,
        objektnummer AS part_1,
        'Objekt Nr Einbauten'::text AS part_3,
        (st_asgeojson(st_envelope(geometrie), 0, 1)::json -> 'bbox'::text)::text AS bbox
    FROM
        afu_wasserbewirtschaftung_pub_v2.wassrbwrtschftung_grundwassereinbau_weitere_einbauten_v
)
SELECT
