...
index_base AS (
    SELECT
        'ch.so.avt.kantonsstrassen_bezugspunkte'::text AS subclass,
        t_id AS id_in_class,
        concat('Nr: ', achsenumme, '-', bezeichnun, ' (Kantonsstrasse Bezugspunkt)') AS displaytext,
        concat(achsenumme, '-', bezeichnun) AS part_1,
        'Kantonsstrasse Nr Bezeichnung'::text AS part_3,
        (st_asgeojson(st_envelope(geometrie), 0, 1)::json -> 'bbox'::text)::text AS bbox
    FROM
        avt_kantonsstrassen_pub.bezugspunkt
    WHERE
        bezugspunkt.achsenumme IS NOT NULL
)
SELECT
